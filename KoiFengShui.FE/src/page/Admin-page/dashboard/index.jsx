...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    endpoints.forEach(({ url, key }) => fetchData(url, (newData) => {
  ...
  const userChartData = createChartData(
    Object.keys(countItems(data.ads.filter(ad => ad.status === "Approved"), 'userId')),
    Object.values(countItems(data.ads.filter(ad => ad.status === "Approved"), 'userId')),
    "Số lượng quảng cáo",
  ...
  const renderChart = (condition, ChartComponent, chartData, options, index) => {
    console.log(`Rendering chart ${index}:`, { condition, chartData });
    if (!condition) {
      console.log(`Chart ${index} not rendered due to condition:`, condition);
      return <p>Loading data...</p>;
    }
    return <ChartComponent data={chartData} options={options} ref={el => chartRefs.current[index] = el} />;
  };
      ...
      <div className="dashboard-content">
        <div className="overview-cards">
          ...
          <div className="overview-card" style={{ backgroundColor: '#ff9800' }}>
            ...
            <p>{Object.values(data.revenue).reduce((acc, curr) => acc + curr, 0)}</p>
          </div>
        </div>
        <div className="chart-container">
          {renderChart(Object.keys(data.revenue).length > 0, Pie, revenueChartData, chartOptions, 0)}
        </div>
        <div className="chart-container">
          {renderChart(Object.keys(data.ageGroups).length > 0, Bar, createChartData(Object.keys(data.ageGroups), Object.values(data.ageGroups), "Số lượng người dùng theo nhóm tuổi", { bg: "rgba(153, 102, 255, 0.6)", border: "rgba(153, 102, 255, 1)" }), { ...chartOptions, plugins: { ...chartOptions.plugins, title: { display: true, text: "Số lượng người dùng theo nhóm tuổi" } } }, 1)}
        </div>
        <div className="chart-container">
          {renderChart(data.ads.length > 0, Bar, userChartData, { ...chartOptions, plugins: { ...chartOptions.plugins, title: { display: true, text: "Số lượng quảng cáo theo người dùng" } } }, 2)}
        </div>
        <div className="chart-container">
          {renderChart(data.fish.length > 0, Bar, fishChartData, { ...chartOptions, plugins: { ...chartOptions.plugins, title: { display: true, text: "Số lượng cá theo mệnh" } } }, 3)}
        </div>
        <div className="chart-container" style={{ height: '400px' }}>
          {data.monthlyRevenue && Object.keys(data.monthlyRevenue).length > 0 ? (
            renderChart(true, Line, monthlyRevenueChartData, {
              ...chartOptions,
              maintainAspectRatio: false,
              plugins: {
                ...chartOptions.plugins,
                title: { display: true, text: `Tổng doanh thu theo tháng năm ${currentYear}` }
              },
              scales: {
                y: {
                  beginAtZero: true,
